  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <ScreenHeader title="Gestion Budget" subtitle="Réorganisation des piliers" />

        {/* --- CURRENT STATE --- */}
        <View style={styles.stateRow}>
          <Card style={styles.stateCard}>
            <Text style={styles.stateLabel}>Essentiel</Text>
            <Text style={styles.stateValue}>{balances.total_essential?.toFixed(0)} $</Text>
          </Card>
          <Card style={styles.stateCard}>
            <Text style={styles.stateLabel}>Loisir</Text>
            <Text style={styles.stateValue}>{balances.total_personal?.toFixed(0)} $</Text>
          </Card>
          <Card style={styles.stateCard}>
            <Text style={styles.stateLabel}>Épargne</Text>
            <Text style={styles.stateValue}>{balances.total_investment?.toFixed(0)} $</Text>
          </Card>
        </View>
        ...
        <Card style={styles.mainCard}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle}>Migration Manuelle</Text>
          </View>
          
          <Text style={styles.label}>Transférer de :</Text>
          <View style={styles.pickerRow}>
            {['essential', 'personal', 'investment'].map((cat: any) => (
              <TouchableOpacity 
                key={cat} 
                onPress={() => setFrom(cat)}
                style={[styles.pickerBtn, from === cat && styles.pickerBtnActive]}
              >
                <Text style={[styles.pickerBtnText, from === cat && {color: '#FFF'}]}>
          ...
          <View style={styles.pickerRow}>
            {['essential', 'personal', 'investment'].map((cat: any) => (
              <TouchableOpacity 
                key={cat} 
                onPress={() => setTo(cat)}
                style={[styles.pickerBtn, to === cat && styles.pickerBtnActive]}
              >
                <Text style={[styles.pickerBtnText, to === cat && {color: '#FFF'}]}>
                  {cat === 'essential' ? 'Essentiel' : (cat === 'personal' ? 'Loisir' : 'Épargne')}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <Text style={styles.label}>Montant à déplacer ($)</Text>
          <TextInput 
            style={styles.input} 
            placeholder="0.00" 
          ...
          <TouchableOpacity style={styles.primaryBtn} onPress={handleMigrate}>
        ...
        <Card style={[styles.mainCard, { borderColor: Colors.warning + '40', borderWidth: 1 }]}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle}>Ré-équilibrage 30/30/40</Text>
          </View>
        ...
        >
          ...
          <Text style={styles.historyLinkText}>Voir l'historique des alertes (Corbeille)</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  stateLabel: { fontSize: 10, color: Colors.textSecondary, fontWeight: '800', textTransform: 'uppercase' },
  stateValue: { fontSize: 16, fontWeight: '800', color: Colors.ink, marginTop: 4 },
  ...
  label: { fontSize: 13, fontWeight: '700', color: Colors.textSecondary, marginBottom: 10, marginTop: 15 },
  
  pickerRow: { flexDirection: 'row', gap: 8 },
  pickerBtn: { flex: 1, paddingVertical: 10, borderRadius: 12, backgroundColor: '#F3F4F6', alignItems: 'center', borderWidth: 1, borderColor: '#E5E7EB' },
  pickerBtnActive: { backgroundColor: Colors.accent, borderColor: Colors.accent },
  pickerBtnText: { fontSize: 11, fontWeight: '800', color: Colors.textSecondary },
  
  input: { height: 50, backgroundColor: '#F3F4F6', borderRadius: 12, paddingHorizontal: 15, fontSize: 18, fontWeight: '700', color: Colors.ink },
  primaryBtn: { height: 54, backgroundColor: Colors.ink, borderRadius: 16, justifyContent: 'center', alignItems: 'center', marginTop: 25 },
  primaryBtnText: { color: '#FFF', fontSize: 16, fontWeight: '800' },
  
  descText: { fontSize: 13, color: Colors.textSecondary, lineHeight: 20, marginBottom: 10, fontWeight: '500' },
  historyLink: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 20 },
  historyLinkText: { fontSize: 13, color: Colors.textSecondary, fontWeight: '600' }
});
